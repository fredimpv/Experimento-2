...
Parte2Chat.prototype.iAmHere = function() {
 ...
 var numerogrupo = firebase.database().ref('/experiment/'+Parte2Chat.experimentoChave+'/participant/' + QueryString.k + '/group/' + 'number').once('value').then(function(snapshot) {
       var group_number = snapshot.val();
       console.log('Numero do Grupo : ' + group_number);


       var adc = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat/' + group_number).once('value').then(function(snapshot) {
       var adc_number = snapshot.val();
       console.log('Inicial + 1   : ' + adc_number);
       var maisum = adc_number + 1 ;

       var acrescenta = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat/' + group_number);
        acrescenta.set(maisum);

      var ref = firebase.database().ref('/experiment/'+ Parte2Chat.experimentoChave + '/contadorchat/' + group_number);
